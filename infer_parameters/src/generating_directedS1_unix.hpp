...
#include <unistd.h>
// embeddingS1
#include "generating_directedS1.hpp"
...
//   required.
// =~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=
bool parse_options(int argc , char *argv[], generating_directedS1_t &the_graph)
{
  ...
  // Parsing options.
  int opt;
  while ((opt = getopt(argc,argv,"ab:i:hm:no:s:tv")) != -1)
  {
    switch(opt)
    {
      case 'a':
      ...
      case 'o':
        ...
        the_graph.CUSTOM_OUTPUT_ROOTNAME = true;
        break;

      case 's':
        the_graph.SEED = std::stoi(optarg);
        the_graph.engine.seed(the_graph.SEED);
        break;
      ...
      case 'v':
        the_graph.SAVE_COORDINATES = true;
        break;
  ...
  if(the_graph.CUSTOM_OUTPUT_ROOTNAME == false)
  {
    size_t lastdot = the_graph.HIDDEN_VARIABLES_FILENAME.find_last_of("_");
