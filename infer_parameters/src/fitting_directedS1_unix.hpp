...
#include <unistd.h>
// embeddingS1
#include "fitting_directedS1.hpp"
...
void print_help()
{
  ...
  std::cout << "\t-n [VALUE]     Specify the value for nu to be used for the embedding. By "        << std::endl;
  std::cout << "\t               default the program infers the value of nu based on the"           << std::endl;
  std::cout << "\t               reciprocity of the original edgelist."                             << std::endl;
...
// =~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=
void parse_options(int argc , char *argv[], fitting_directedS1_t &the_graph)
{
  ...
  // Parsing options.
  int opt;
  while ((opt = getopt(argc,argv,"ab:cn:o:qs:")) != -1)
  {
    switch(opt)
    {
      case 'a':
      ...
      case 'n':
        the_graph.CUSTOM_NU = true;
        the_graph.nu = std::stod(optarg);
        break;
