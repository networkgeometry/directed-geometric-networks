...
if __name__ == "__main__":
    ...
    vnames = np.array(['v' + str(i).zfill(4) for i in range(nb_vertices)])
    ...
    # Creates a dataframe with everything in it and saves it in a file.
    df = pd.DataFrame(data={'vertex': vnames, 'kappa_in': kappa, 'kappa_out': kappa, 'theta': theta})
    df.to_fwf('../../directedS1_data/synthetic_networks/hidden_variables/graphA_00_hidden_variables.txt', ['vertex', 'kappa_in', 'kappa_out', 'theta'])
    ...
    np.random.shuffle(kappa)
    df['kappa_out'] = kappa
    df.to_fwf('../../directedS1_data/synthetic_networks/hidden_variables/graphB_00_hidden_variables.txt', ['vertex', 'kappa_in', 'kappa_out', 'theta'])
    ...
    # # Creates a dataframe with everything in it and saves it in a file.
    # df = pd.DataFrame(data={'vertex': vnames, 'kappa_in': kappa, 'kappa_out': kappa, 'theta': theta})
    # df.to_fwf('../../directedS1_data/synthetic_networks/hidden_variables/graphE_00_hidden_variables.txt', ['vertex', 'kappa_in', 'kappa_out', 'theta'])
    ...
    # np.random.shuffle(kappa)
    # df['kappa_out'] = kappa
    # df.to_fwf('../../directedS1_data/synthetic_networks/hidden_variables/graphF_00_hidden_variables.txt', ['vertex', 'kappa_in', 'kappa_out', 'theta'])