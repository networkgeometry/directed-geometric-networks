...
for i in range(numberInferenceRuns):
    ...
    outputFilename = "../../directedS1_data/real_networks/inferred_parameters/" + networkName + "_nu{}_{:02d}".format(nu.replace(".","p").replace("-","m"), i)
    if not os.path.isfile(outputFilename + "_log.txt"):
        cmd = ["../infer_parameters/fitting_directedS1", "-o", outputFilename, "-n", nu, edgelistFilename]
        # print(cmd)
